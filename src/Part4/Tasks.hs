...
instance Applicative ReverseList where
    pure x = REmpty :< x
    (<*>) fs lst = getReversed REmpty $ applyReverseLists REmpty fs lst
        where
            applyReverseLists acc REmpty _ = acc
            applyReverseLists acc (fs :< f) lst = applyReverseLists (fmapReverseList acc f lst) fs lst
            fmapReverseList acc _ REmpty = acc
            fmapReverseList acc f (xs :< x) = fmapReverseList (acc :< f x) f xs
            getReversed acc REmpty = acc
            getReversed acc (xs :< x) = getReversed (acc :< x) xs
...
instance Monad ReverseList where
    return x = REmpty :< x
    (>>) m k = m >>= \_ -> k
    (>>=) lst f = getReversed REmpty $ bindReverseLists REmpty lst f
        where
            bindReverseLists acc REmpty _ = acc
            bindReverseLists acc (lst :< x) f = bindReverseLists (mappendReverseList acc $ f x) lst f
            mappendReverseList acc REmpty = acc
            mappendReverseList acc (xs :< x) = mappendReverseList (acc :< x) xs
            getReversed acc REmpty = acc
            getReversed acc (xs :< x) = getReversed (acc :< x) xs